...
If you deploy this repository directly to Vercel, the root URL now serves `index.html` to avoid root-level 404s.

Current status:
- ✅ Static docs + landing page deploy correctly to Vercel.
- ⚠️ The pilot API in `app/main.py` is a long-running server process and is intended for local/staging runtime, not serverless Vercel Functions in its current form.

For production Vercel API hosting, the next step is to port endpoints to Vercel Functions (or deploy the Python API to a container/runtime service and keep Vercel for frontend).
